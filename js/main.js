...
/* función constructora para los clientes -  relaciona el mail con las actividades del carrito */
...
/* agregado de las cards una vez que el usuario se logea las podrá ver, las lee del json con fetch */

const card_gen = document.querySelector("#contenedor_cards")
...
fetch('../data/actividades.json')
    ...
    .then((data) => {
        ...
        data.forEach((item) => {
            ...
            division.classList.add('col-md-4', 'mb-3', 'mx-auto');
            division.innerHTML = `
                <div class="card">
                <img src="${item.ruta_imagen}" class="card-img-top" alt="...">
                <div class="card-body ">
                    <h5 class="card-title">${item.actividad}</h5>
                    <h6 class="card-title">Por mes: $<span>${item.precio}</span></h6>
                    <p class="card-text">${item.descripcion.replace(/\n/g, '<br>')}</p>
                    <div class="d-grid gap-2">
                        <button class="add-to-cart btn btn-primary" data-product-id="1"
                            data-product-name="${item.actividad}">Agregar al carrito</button>
                    </div>
                </div>
            </div>
            `
            card_gen.append(division)
        })
    })
...
function readTheContent(product) {
    const infoProduct = {
        ...
        price: product.querySelector('div h6 span').textContent,
    }
    ...
    if (index !== -1) {
        Toastify({
            text: "Ya agregó esta actividad al carrito",
            duration: 2000,
            gravity: "top", 
            position: "center", 
            stopOnFocus: false, 
    ...
    Toastify({
        text: `${infoProduct.title} agregado al carrito 🛒`,
        duration: 1500,
        stopOnFocus: false, // para que por más que me pare en el cartel el timer siga
        style: {
            background: "#cc154e",
        },
        offset: {
...
function loadHtml() {
    ...
    compras_en_carrito.forEach(product => {
        ...
        row.innerHTML = `
        <div class="div_line2">
            <div><span class="delete-product btn-close" data-id="${title}"></span></div>
